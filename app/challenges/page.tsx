...
export default function ChallengesPage() {
  ...
  const ChallengeCard = ({ challenge, index }: { challenge: ClientChallenge; index: number }) => {
    return (
      <motion.div
      ...
      >
        ...
        >
          <div className={`${viewMode === 'grid' ? 'flex flex-col' : 'flex items-center justify-between'}`}>
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {challenge.skills.length > (viewMode === 'grid' ? 4 : 3) && (
                    <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-50 text-gray-500">
                      +{challenge.skills.length - (viewMode === 'grid' ? 4 : 3)} more
                    </span>
                  )}
                </div>
              </div>
            ...
            {filteredChallenges.length === 0 ? (
              ...
              >
                {filteredChallenges.map((challenge: ClientChallenge, index: number) => (
                  <ChallengeCard key={challenge.slug} challenge={challenge} index={index} />
                ))}
              </motion.div>
            )}
          </AnimatePresence>
